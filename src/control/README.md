...
In order to run the shell to perform administrative tasks, build and run the `daos_server` as per the [quickstart guide](https://github.com/daos-stack/daos/blob/master/doc/quickstart.html).
...
Operations on NVMe SSD devices are performed using [go-spdk bindings](./go-spdk/README.html) to issue commands over the SPDK framework.
