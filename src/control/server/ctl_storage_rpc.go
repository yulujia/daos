...
func (c *StorageControlService) doNvmePrepare(req *pb.PrepareNvmeReq) (resp *pb.PrepareNvmeResp) {
	...
	if err != nil {
		resp.State = storage.NewState(c.log, pb.ResponseStatus_CTRL_ERR_NVME, err.Error(), "", msg)
		return
	}

	resp.State = storage.NewState(c.log, pb.ResponseStatus_CTRL_SUCCESS, "", "", msg)
	return
}
...
func (c *StorageControlService) doScmPrepare(req *pb.PrepareScmReq) (resp *pb.PrepareScmResp) {
	...
	if err != nil {
		resp.State = storage.NewState(c.log, pb.ResponseStatus_CTRL_ERR_SCM, err.Error(), "", msg)
		return
	}
	...
	resp.State = storage.NewState(c.log, pb.ResponseStatus_CTRL_SUCCESS, "", info, msg)
	...
	*pb.StorageScanResp, error) {
	...
	if err != nil {
		resp.Nvme = &pb.ScanNvmeResp{
			State: storage.NewState(c.log, pb.ResponseStatus_CTRL_ERR_NVME, err.Error(), "", msg+"NVMe"),
		}
	} else {
		resp.Nvme = &pb.ScanNvmeResp{
			State:  storage.NewState(c.log, pb.ResponseStatus_CTRL_SUCCESS, "", "", msg+"NVMe"),
			Ctrlrs: controllers,
		}
	}
	...
	if err != nil {
		resp.Scm = &pb.ScanScmResp{
			State: storage.NewState(c.log, pb.ResponseStatus_CTRL_ERR_SCM, err.Error(), "", msg+"SCM"),
		}
	} else {
		resp.Scm = &pb.ScanScmResp{
			State:   storage.NewState(c.log, pb.ResponseStatus_CTRL_SUCCESS, "", "", msg+"SCM"),
			Modules: modules,
			Pmems:   pmemDevs,
		}
	}
