	       ...
	       uint32_t *map_version, struct rsvc_hint *hint)
{
	...
	} else if (updated && opc == POOL_ADD) {
		int	 ret;

		/* TODO: Environment variable / config setting to disable add */
		D_ASSERT(replicas != NULL);
		ret = ds_rebuild_schedule(pool_uuid, *map_version, NULL,
